...
if ([string]::IsNullOrWhiteSpace($BuildDir)) {
	$BuildDir = Join-Path ([System.IO.Path]::GetTempPath()) "ofxGgmlDiffusion-tests"
}
...
if (Test-WindowsHost) {
	...
	Write-Step "Configuring and running ofxGgmlDiffusion tests with Visual Studio tools"
	Invoke-CheckedCmd "ofxGgmlDiffusion tests" $command
} else {
	Write-Step "Configuring ofxGgmlDiffusion tests"
	Invoke-CheckedNative "cmake configure ofxGgmlDiffusion tests" {
		cmake -S $testsDir -B $BuildDir -DCMAKE_BUILD_TYPE=$Configuration
	}
	Write-Step "Building ofxGgmlDiffusion tests"
	Invoke-CheckedNative "cmake build ofxGgmlDiffusion tests" {
		cmake --build $BuildDir --config $Configuration
	}
	Write-Step "Running ofxGgmlDiffusion tests"
	Invoke-CheckedNative "ctest ofxGgmlDiffusion tests" {
